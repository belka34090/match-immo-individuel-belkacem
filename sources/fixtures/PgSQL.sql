...
-- Cible : PostgreSQL (stack du cours, port 5433, base DB_Cours)
-- Rejouable : DROP SCHEMA + CREATE + INSERT
--
...
DROP SCHEMA IF EXISTS "Fil_Rouge_Depart" CASCADE;
CREATE SCHEMA "Fil_Rouge_Depart";
SET search_path TO "Fil_Rouge_Depart";

-- Sous PostgreSQL, ENUM = un TYPE nommé, réutilisable
CREATE TYPE role_utilisateur AS ENUM ('client', 'chasseur');
CREATE TYPE statut_mandat    AS ENUM ('actif', 'suspendu', 'termine', 'expire');
...
CREATE TABLE utilisateurs (
    ...
    ville VARCHAR(80),
    taux_commission NUMERIC(4,2),
    budget_max NUMERIC(12,2),
    date_creation DATE NOT NULL
);

COMMENT ON COLUMN utilisateurs.taux_commission IS 'chasseurs uniquement (%)';
COMMENT ON COLUMN utilisateurs.budget_max      IS 'clients uniquement (EUR)';
...
CREATE TABLE mandats (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    client_id INT NOT NULL REFERENCES utilisateurs(id),
    chasseur_id INT NOT NULL REFERENCES utilisateurs(id),
    secteur_id INT REFERENCES secteurs(id),
    exclusif BOOLEAN NOT NULL DEFAULT FALSE,
    date_debut DATE NOT NULL,
    statut statut_mandat NOT NULL DEFAULT 'actif',
...
( 7, 1, 1, TRUE,  '2025-02-01', 'termine', 'T3 Ecusson, budget 320000, 65m2 min, balcon, calme, DPE C max'),
( 8, 3, 7, FALSE, '2025-03-10', 'expire',  'T4 Croix-Rousse, budget 450000, 85m2, terrasse ou jardin'),
( 9, 1, 3, FALSE, '2025-04-05', 'termine', 'T2 Beaux-Arts, budget 280000, 45m2 min, lumineux, proche tram'),
(10, 4, 9, TRUE,  '2025-05-20', 'actif',   'Maison Ile de Nantes, budget 390000, 3 chambres, petit exterieur'),
(11, 2, 2, TRUE,  '2025-06-18', 'termine', 'T4 Port Marianne, budget 550000, 90m2, parking, ascenseur, vue'),
(12, 5, 5, FALSE, '2025-07-22', 'expire',  'Maison Castelnau, budget 240000, 80m2, jardin, travaux OK'),
(13, 3, 8, TRUE,  '2025-09-01', 'actif',   'Loft Confluence, budget 610000, 100m2, standing, terrasse'),
(14, 2, 1, FALSE, '2025-09-15', 'suspendu','T3 Ecusson ou Beaux-Arts, budget 300000, charme ancien, poutres'),
(15, 6,10, TRUE,  '2025-10-02', 'actif',   'T3 Sete centre, budget 260000, vue mer si possible, 60m2'),
(16, 5, 6, FALSE, '2025-11-14', 'actif',   'Villa Lattes, budget 420000, 4 pieces, piscine ou jardin sud'),
(17, 1, 2, TRUE,  '2026-01-05', 'actif',   'T3 Port Marianne, budget 350000, neuf ou recent, balcon, parking'),
(18, 4, 9, FALSE, '2026-01-20', 'actif',   'Appartement Nantes, budget 480000, 4 pieces, dernier etage'),
...
( 3, 2, 4, FALSE, '2026-02-10', 'actif',   'T2 Figuerolles, budget 220000, premier achat, 40m2 min'),
(20, 3, 8, TRUE,  '2026-03-01', 'actif',   'T5 Confluence, budget 700000, 120m2, prestations haut de gamme'),
(21, 1, 1, FALSE, '2026-03-25', 'actif',   'T3 Ecusson, budget 310000, ancien renove, cave appreciee'),
(22, 6,10, FALSE, '2026-04-12', 'suspendu','Maison Sete, budget 290000, 3 pieces, garage'),
(23, 5, 3, TRUE,  '2026-05-28', 'actif',   'T2 Beaux-Arts, budget 260000, 45m2, balcon, DPE D max'),
(24, 2, 5, TRUE,  '2026-07-01', 'actif',   'Maison Castelnau, budget 330000, 90m2, 3 chambres, jardin');
...
-- [consultant] (date_debut + INTERVAL '6 months' < aujourd'hui). Le statut
-- [consultant] "actif" ne veut donc pas dire grand-chose en l'état.
--
-- 💡 Rappel : ce jeu de données vit dans le schéma "Fil_Rouge_Depart".
--    SET search_path TO "Fil_Rouge_Depart";
