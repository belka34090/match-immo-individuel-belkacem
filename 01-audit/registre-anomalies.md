...
| R-11 | Rémunération des chasseurs | Dans l’ancienne base, chaque chasseur possède seulement un taux de commission unique. Or la rémunération réelle dépend de plusieurs éléments : la période concernée, le montant de la vente, l’ancienneté du chasseur et sa performance. L’ancien système ne peut donc pas représenter correctement toutes les règles prévues par le métier. | L’entreprise ne peut pas expliquer de manière fiable pourquoi un chasseur reçoit tel montant, ni garantir qu’un ancien calcul restera identique si les règles changent plus tard. | Élevée | Prévoir dans la future base des règles de rémunération détaillées et datées, ainsi que les informations utilisées pour calculer et justifier le montant versé au chasseur. | Besoin métier du Starter Pack et `analyse-audit.md` |
